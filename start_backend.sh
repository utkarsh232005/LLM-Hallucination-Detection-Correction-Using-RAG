...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./start_backend.sh [--install]

Options:
  --install   Install/refresh Python dependencies from requirements.txt before start
...
if ! command -v lsof >/dev/null 2>&1; then
  ...
  if lsof -iTCP:"$PORT" -sTCP:LISTEN -t >/dev/null 2>&1; then
    ...
    echo "[info] If this is an old app process, stop it with: lsof -ti:$PORT | xargs kill -9"
    exit 1
  fi
fi
